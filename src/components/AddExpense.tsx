    ...
    <div className="max-w-md mx-auto">
      ...
      >
        <div className="relative h-32 bg-gradient-to-r from-rose-500 to-pink-500 px-6 pt-6">
          ...
          <h2 className="mt-4 text-2xl font-bold text-white">Add Expense</h2>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="relative mt-2">
            ...
            />
            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400">ETB</span>
          </div>
          {errors.amount && <p className="text-sm text-rose-500 mt-2">{errors.amount.message}</p>}
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {quickAmounts.map((value) => (
              <button
                key={value}
                type="button"
                onClick={() => setValue("amount", (Number(currentAmount) || 0) + value, { shouldValidate: true })}
              ...
              return (
                <button
                  ...
                  className={`p-3 rounded-xl text-center transition ${
                    active ? `bg-gradient-to-r ${cat.color} text-white shadow-lg` : "bg-slate-100 dark:bg-slate-700 text-slate-500"
                  }`}
                >
        ...
        <div className="px-6 pb-6">
          <label className="text-xs text-slate-400 mb-2 block">Note</label>
          <div className="relative">
            ...
            />
            <Wallet size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400" />
          </div>
          {errors.title && <p className="text-sm text-rose-500 mt-2">{errors.title.message}</p>}
        </div>
